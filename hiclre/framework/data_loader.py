...
class BagREDataset(data.Dataset):
    ...
    def lookup_discription_tokenAndAttMask(self, bag_name, candidate_token, candidate_attMask, entityid2tokenAttMask_dictFormat):
        candidate_token = candidate_token.numpy().tolist()[0]
        candidate_attMask = candidate_attMask.numpy().tolist()[0]
        head_id = bag_name[0]
        tail_id = bag_name[1]
        head_tokenAttMask = entityid2tokenAttMask_dictFormat.get(head_id)
        tail_tokenAttMask = entityid2tokenAttMask_dictFormat.get(tail_id)
        if head_tokenAttMask is not None:
            head_token = head_tokenAttMask['input_ids']
            head_attMask = head_tokenAttMask['attention_mask']
        else:
        ...
        t_t = torch.tensor(tail_token).unsqueeze(0)
        h_a = torch.tensor(head_attMask).unsqueeze(0)
        t_a = torch.tensor(tail_attMask).unsqueeze(0)
        discription_tokens = torch.cat((h_t, t_t),0)
        discription_attMask = torch.cat((h_a, t_a),0)

        return discription_tokens, discription_attMask
    ...
    def wiki_discription(self):
        ...
        for item in wiki_discription:
            entity_id = item.split('\t')[0]
            discription = item.split('\t')[-1]
            discription_dictFormat[entity_id] = discription
        return discription_dictFormat
...
class WikiEntityOrDiscriptionDataset(data.Dataset):
...
def WikiDataLoader(path, shuffle, num_workers=8,):

    dataset = WikiEntityOrDiscriptionDataset(path)
    data_loader = data.DataLoader(dataset=dataset,
            shuffle=shuffle,
            pin_memory=True,
            num_workers=num_workers)
